setwd('/Users/NeilWu/Github/Research_LungSCC_prognosis/GSE29013_main/')
GSE_data<-read.table('GSE29013_series_matrix.txt',skip = 44,fill = T,sep='\t',stringsAsFactors = F,
                     header = T)

GSE_clinical<-GSE_data[c(16,15,11),]
GSE_clinical[2,]<-grepl('death_event: 1',GSE_clinical[2,])
GSE_clinical[1,]<-sapply(GSE_clinical[1,],USE.NAMES = F,
                         function(x) strsplit(x,'death_time: ')[[1]][2])
GSE_clinical[3,]<-grepl('histology: Squamous',GSE_clinical[3,])
...
write.table(GSE_data_merged_sq,'Data_merged_29013.txt',sep='\t',quote = F)
