GSE_data<-read.table('GSE8894_series_matrix.txt',skip = 44,header=T,
                     fill = T,sep='\t',stringsAsFactors = F)

GSE_clinical<-GSE_data[c(8,12,11),]
GSE_clinical[1,]<-sapply(GSE_clinical[1,],USE.NAMES = F,
                         function(x) strsplit(x,': ')[[1]][2])
GSE_clinical[2,]<-sapply(GSE_clinical[2,],USE.NAMES = F,
                         function(x) strsplit(x,': ')[[1]][2])
GSE_clinical[3,]<-grepl('squamous',GSE_clinical[3,])
GSE_clinical<-data.frame(t(GSE_clinical[,-1]))
colnames(GSE_clinical)<-c('event_death','day_to_event_death','Subtype_sq')
...
write.table(GSE_data_merged_sq,'Data_merged_8894.txt',sep='\t',quote = F)
