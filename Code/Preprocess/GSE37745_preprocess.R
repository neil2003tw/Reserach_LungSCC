GSE_data<-read.table('GSE37745_series_matrix.txt',skip = 45,fill = T,sep='\t',stringsAsFactors = F,
                     header = T)

GSE_clinical<-GSE_data[c(9,8,10),]
GSE_clinical[2,]<-grepl('yes',GSE_clinical[2,])
GSE_clinical[1,]<-sapply(GSE_clinical[1,],USE.NAMES = F,
                         function(x) strsplit(x,'days to determined death status: ')[[1]][2])
GSE_clinical[3,]<-grepl('squamous',GSE_clinical[3,])
GSE_clinical<-data.frame(t(GSE_clinical[,-1]))
colnames(GSE_clinical)<-c('day_to_event_death','event_death','Subtype_sq')

GSE_data<-GSE_data[-c(1:40),]
...
GSE_data_merged_sq<-GSE_data_all[ as.character(GSE_data_all$Subtype_sq)=='FALSE',]
GSE_data_merged_sq<-GSE_data_merged_sq[,-dim(GSE_data_merged_sq)[2]]
write.table(GSE_data_merged_sq,'Data_merged_37745_AD.txt',sep='\t',quote = F)
