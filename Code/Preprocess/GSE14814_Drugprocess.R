...
GSE_clinical<-GSE_data[c(15,16,9),]
...
GSE_clinical[3,]<-grepl('Post Surgical Treatment: ACT',GSE_clinical[3,])

GSE_clinical<-data.frame(t(GSE_clinical[,-1]))
colnames(GSE_clinical)<-c('day_to_event_death','event_death','Chemo')
...
GSE_data_merged_sq<-GSE_data_all[ as.character(GSE_data_all$Chemo)=='TRUE',]
GSE_data_merged_sq<-GSE_data_merged_sq[,-dim(GSE_data_merged_sq)[2]]
write.table(GSE_data_merged_sq,'Data_merged_14814_Chemo.txt',sep='\t',quote = F)

GSE_data_merged_fr<-GSE_data_all[ as.character(GSE_data_all$Chemo)=='FALSE',]
GSE_data_merged_fr<-GSE_data_merged_fr[,-dim(GSE_data_merged_fr)[2]]
write.table(GSE_data_merged_fr,'Data_merged_14814_Chemofree.txt',sep='\t',quote = F)
